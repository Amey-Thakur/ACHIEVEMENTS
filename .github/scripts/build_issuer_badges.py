...
# A stack the renderer will actually have, and one whose members are close
# enough in width that pinning the text to a measured length does not stretch
# it: Verdana on Windows, DejaVu Sans on Linux. The text carries that measured
# textLength, so it fits its segment exactly whichever of them wins.
FONT_STACK = "Verdana,DejaVu Sans,Geneva,sans-serif"

# The names are set in the regular weight, with a soft dark line a pixel below
# to hold them off the colour behind. Bold looked heavier than it read: the
# widths here are measured from the regular face, so bold type was being
# squeezed into a regular width and every name came out slightly condensed.
...
<text x="{round(left + count_w / 2, 1)}" y="{shadow}" text-anchor="middle" \
textLength="{round(tally_w, 1)}" lengthAdjust="spacingAndGlyphs" fill="#010101" \
fill-opacity=".3">{escape(tally)}</text>
<text x="{round(left + count_w / 2, 1)}" y="{baseline}" text-anchor="middle" \
textLength="{round(tally_w, 1)}" lengthAdjust="spacingAndGlyphs" \
fill="#ffffff">{escape(tally)}</text>
</g>
</svg>
'''
